...
void difftest_step(uint32_t eip) {
  ...
  if (diff) {
    nemu_state = NEMU_END;
	printf("ERROR!\n");
	printf(" NEMU.eax=0x%08x\n QEMU.eax=0x%08x\n",cpu.eax,r.eax);
    printf(" NEMU.ecx=0x%08x\n QEMU.ecx=0x%08x\n",cpu.ecx,r.ecx);
	printf(" NEMU.edx=0x%08x\n QEMU.edx=0x%08x\n",cpu.edx,r.edx);
	printf(" NEMU.ebx=0x%08x\n QEMU.ebx=0x%08x\n",cpu.ebx,r.ebx);
	printf(" NEMU.ebp=0x%08x\n QEMU.ebp=0x%08x\n",cpu.ebp,r.ebp);
	printf(" NEMU.esp=0x%08x\n QEMU.esp=0x%08x\n",cpu.esp,r.esp);
	printf(" NEMU.edi=0x%08x\n QEMU.edi=0x%08x\n",cpu.edi,r.edi);
	printf(" NEMU.esi=0x%08x\n QEMU.esi=0x%08x\n",cpu.esi,r.esi);
	printf(" NEMU.eip=0x%08x\n QEMU.eip=0x%08x\n",cpu.eip,r.eip);
  }
}
